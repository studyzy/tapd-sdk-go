...
TAPD Open API 共提供约 200+ 个接口，覆盖 20+ 个模块。本 SDK 经全量审计后已基本覆盖官方文档所有公开接口。

## 功能概览

SDK 覆盖 20+ 种 TAPD 资源类型，提供 **245 个 API 方法**：

| 资源 | 方法数 | 支持操作 |
|------|--------|----------|
| **Story（需求）** | 35 | CRUD、计数、变更、模板、复制、批量更新、视图查询、分类、链接关系、时间关系、转换工作项类型、保密需求、QueryToken、并行工作流节点等 |
| **Bug（缺陷）** | 13 | CRUD、计数、链接缺陷、复制、批量更新、模板、IDs 转 token、相关需求、过滤转 QueryToken 等 |
| **Task（任务）** | 9 | CRUD、计数、批量更新、获取已删除、视图查询、字段信息 |
| **Iteration（迭代）** | 14 | 列表、创建、更新、计数、锁定、解锁、仪表盘卡片、模板列表/字段、类别、计划应用等 |
...
| **Workflow（工作流）** | 7 | 状态流转、状态映射、结束状态、所有结束状态、起始状态、工作流列表、步骤映射 |
| **Change（变更历史）** | 7 | 需求/缺陷/任务变更及计数、迭代变更 |
| **Board（看板）** | 4 | 工作项创建、列表、更新、板块列表 |
| **Release（发布计划）** | 5 | 新发布 CRUD、发布评审列表/更新 |
| **Setting（配置）** | 24 | 模块/版本/基线/特性 CRUD、自定义字段配置、选项更新、级联字段、项目设置、复制类别/缺陷配置 |
| **Workspace（项目）** | 16 | 列表、详情、子项目、公司项目、成员、角色、工作日历、长 ID 转换、文档列表、更新项目信息 |
| **User（用户）** | 4 | 个人配置、第三方用户映射、视图列表、用户信息 |
| **Label（标签）** | 4 | 创建、更新、查询列表、计数 |
| **Pipeline（流水线）** | 3 | 创建关联、查询关联、删除关联 |
...
├── user.go                # 用户 API（个人配置、第三方映射、视图、用户信息）
├── program.go             # 项目集 API
├── label.go               # 标签 API（创建、更新、查询、计数）
├── pipeline.go            # 流水线 API（创建/查询/删除关联）
├── storage.go             # 公共存储 API（保存、查询、更新、删除）
...
└── model/                 # 数据模型与请求参数
    ...
    ├── user.go                   # PersonalSetting、ThirdUserMapping、UserView、UserInfo
    ├── program.go                # 项目集请求/响应
    ├── label.go                  # LabelPool 及标签请求结构体
    ├── pipeline.go               # ThirdRelation 及流水线请求结构体
    ├── storage.go                # 公共存储请求结构体
...
|-----------|-----------|-----------|----------|
| 需求 (Story) | 44 | 35 | CRUD、变更、模板、分类、链接、批量、复制、视图查询、转换工作项类型、保密需求、QueryToken、并行工作流节点 |
| 缺陷 (Bug) | 22 | 13 | CRUD、链接、复制、批量、模板、IDs 转 token、相关需求、过滤转 QueryToken |
| 任务 (Task) | 11 | 9 | CRUD、批量更新、删除项、视图查询、变更、字段信息 |
| 迭代 (Iteration) | 16 | 14 | 列表、创建、更新、计数、锁定、解锁、仪表盘卡片、模板、类别、计划应用 |
...
| 项目 (Workspace) | 18 | 16 | 列表、详情、更新、子项目、成员、角色、工作日历、长 ID、文档 |
| 工作流 (Workflow) | 7 | 7 | 全部覆盖（含步骤映射）|
| 发布 (Release) | 13 | 5 | 新发布 CRUD、发布评审列表/更新 |
| 配置 (Setting) | 24 | 24 | 模块/版本/基线/特性 CRUD、自定义字段配置、选项更新、级联字段、项目设置、复制类别/缺陷配置 |
| 用户 (User) | 5 | 4 | 个人配置、第三方用户映射、视图列表、用户信息 |
| 标签 (Label) | 4 | 4 | 全部覆盖（创建、更新、查询、计数）|
| 流水线 (Pipeline) | 3 | 3 | 全部覆盖（创建/查询/删除关联）|
| 公共存储 (Storage) | 5 | 4 | 保存、查询、更新、删除 |
